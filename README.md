...
当前版本：`0.1.0`<br>
作者：`stlin256`<br>
仓库：`https://github.com/stlin256/GPUFleet`
...
| 设备管理 | 已实现 | 创建设备、一次性密钥、禁用/启用、轮换密钥 |
...
- 设备：创建设备、展示一次性密钥、禁用/启用设备、轮换密钥。
- GPU：完整 GPU 运行字段、2x2 历史趋势图、进程快照和统计。
- 设置：服务状态、密码更改、端口配置、HTTPS 证书上传、证书到期日期、数据库下载、配置引导、作者/仓库、版本号和 Changelog。
...
验证脚本会检查登录、30 天 Cookie 会话恢复、总览卡片、每卡历史图表、悬浮读数、离线蒙版、同设备边框同色、深浅主题、移动端无横向溢出、设置页操作入口、版本号和 Changelog。
