...
- `gpufleet-agent`：Windows/Linux 客户端，使用 `nvidia-smi` 只读采集 NVIDIA GPU 指标并主动上报。
- Web 面板：React + Vite + TypeScript，使用 lucide-react、TanStack Query 和 Apache ECharts，构建产物已纳入 `web/dist`。
...
| 当前前端 | React + Vite + lucide-react + TanStack Query + ECharts | 响应式面板、图标、轮询数据、统计图表 |
