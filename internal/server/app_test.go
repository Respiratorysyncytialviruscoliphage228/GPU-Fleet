...
func TestAdminRuntimeConfigCertificateAndDownload(t *testing.T) {
	...
	doJSON(t, handler, http.MethodPost, "/api/v1/admin/server-config", map[string]int{"min_free_mb": 101}, cookie, http.StatusOK, &reserveUpdate)
	if !reserveUpdate.OK || reserveUpdate.Service.MinFreeBytes != 101*1024*1024 || reserveUpdate.RestartRequired {
	...
	if reserveOverview.MinFreeSpaceBytes != 101*1024*1024 || reserveOverview.Disk.MinFreeBytes != 101*1024*1024 {
	...
	doJSON(t, handler, http.MethodPost, "/api/v1/admin/server-config", map[string]int{"min_free_mb": 99}, cookie, http.StatusBadRequest, nil)
