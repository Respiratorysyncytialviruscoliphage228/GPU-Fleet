...
Official release builds are generated by `scripts/build-release.ps1` and the manually triggered GitHub Release workflow. The default `full` release matrix builds separate Server and Agent packages for Windows, Linux, macOS, and FreeBSD across Go-supported common architectures. Windows/Linux are the supported Agent operating systems for NVIDIA GPU collection; macOS and FreeBSD Agent packages are provided for completeness and diagnostics where `nvidia-smi` is available.

官方发布包由 `scripts/build-release.ps1` 和手动触发的 GitHub Release 工作流生成。默认 `full` 发布矩阵会为 Windows、Linux、macOS 和 FreeBSD 的 Go 常见支持架构分别构建 Server 与 Agent 包。NVIDIA GPU 采集实际支持的 Agent 操作系统仍以 Windows/Linux 为主；macOS 和 FreeBSD Agent 包主要用于完整性和具备 `nvidia-smi` 环境时的诊断。
