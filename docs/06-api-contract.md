...
当相邻采样间隔超过当前范围的保护上限时，该段不累计能耗，避免设备离线或数据缺口被误算成持续耗电。1H 使用原始序列，24H 使用分钟 rollup，7D 使用 15 分钟 rollup，30D 使用小时 rollup。
长范围曲线会在每个展示桶内按 GPU 保留最新点，避免最新快照和 rollup 点落在同一小时桶时重复相加；首尾明显稀疏的长范围边界桶不会出现在曲线中。
...
- `idle_waste`：低利用率同时功率高于空转功率阈值，且累计空转耗电达到 `0.005 kWh` 的展示阈值。
