...
- 服务端支持 React/Vite 构建目录托管，并保留内置 HTML 面板作为回退；fallback 面板也已覆盖总览、设备、GPU、设置、深浅主题、GPU 趋势图和完整服务设置摘要。
- Web 面板支持登录、退出、卡片式多机多卡 GPU Fleet 总览、GPU 详情趋势图、趋势图悬浮读数、同设备 GPU 边框同色、设备管理、GPU 当前状态、GPU 进程快照、24 小时统计、深浅主题切换和服务设置页。
