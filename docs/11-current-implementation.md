...
- Web 面板支持首次配置引导、密码登录、当前浏览器设备 30 天持久会话、退出、卡片式多机多卡 GPU Fleet 总览、GPU 详情趋势图、趋势图悬浮读数、同设备 GPU 边框同色、设备管理、GPU 当前状态、GPU 进程快照、24 小时统计、深浅主题切换、服务设置页、页面/卡片/图表/表单动效和移动端底部固定导航。
...
- 已提供 Chrome/Edge CDP 前端验证脚本，覆盖登录、刷新会话恢复、GPU Fleet 卡片面板、GPU 详情趋势图、深浅主题切换和持久化、设备页、设置页、移动端总览、移动端 GPU 页、移动端底部固定导航、截图和移动端横向溢出检查；演示验证可强制检查 5 张 GPU 卡片、离线蒙版和同设备双卡。
